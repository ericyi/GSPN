...
REGISTER_OP("GroupMaxpool")
    .Input("points: float32")
    .Input("idx: int32")
    .Output("out: float32")
    .Output("max_idx: int32")
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
        ::tensorflow::shape_inference::ShapeHandle dims1; // batch_size * ndataset * channels
        c->WithRank(c->input(0), 3, &dims1);
        ::tensorflow::shape_inference::ShapeHandle dims2; // batch_size * npoints * nsample
        c->WithRank(c->input(1), 3, &dims2);
        // batch_size * npoints *  channels
        ::tensorflow::shape_inference::ShapeHandle output = c->MakeShape({c->Dim(dims2, 0), c->Dim(dims2, 1), c->Dim(dims1, 2)});
        ::tensorflow::shape_inference::ShapeHandle output1 = c->MakeShape({c->Dim(dims2, 0), c->Dim(dims2, 1), c->Dim(dims1, 2)});
        c->set_output(0, output);
        c->set_output(1, output1);
        return Status::OK();
    });
REGISTER_OP("GroupMaxpoolGrad")
    .Input("points: float32")
    .Input("max_idx: int32")
    .Input("grad_out: float32")
    .Output("grad_points: float32")
...
class GroupMaxpoolGpuOp: public OpKernel{
    public:
        explicit GroupMaxpoolGpuOp(OpKernelConstruction * context):OpKernel(context){}

        void Compute(OpKernelContext * context) override {
            const Tensor& points_tensor=context->input(0);
            OP_REQUIRES(context, points_tensor.dims()==3, errors::InvalidArgument("GroupMaxpool expects (batch_size, num_points, channel) points shape"));
            int b = points_tensor.shape().dim_size(0);
            int n = points_tensor.shape().dim_size(1);
            int c = points_tensor.shape().dim_size(2);

            const Tensor& idx_tensor=context->input(1);
            OP_REQUIRES(context,idx_tensor.dims()==3 && idx_tensor.shape().dim_size(0)==b, errors::InvalidArgument("GroupMaxpool expects (batch_size, npoints, nsample) idx shape"));
            int m = idx_tensor.shape().dim_size(1);
            int nsample = idx_tensor.shape().dim_size(2);
            ...
            Tensor * max_idx_tensor = nullptr;
            OP_REQUIRES_OK(context, context->allocate_output(1,TensorShape{b,m,c}, &max_idx_tensor));
            ...
            auto out_flat = out_tensor->flat<float>();
            float *out = &(out_flat(0));
            auto max_idx_flat = max_idx_tensor->flat<int>();
            int *max_idx = &(max_idx_flat(0));
            groupMaxpoolLauncher(b,n,c,m,nsample,points,idx,out,max_idx);
        }
};
REGISTER_KERNEL_BUILDER(Name("GroupMaxpool").Device(DEVICE_GPU),GroupMaxpoolGpuOp);

void groupMaxpoolGradLauncher(int b, int n, int c, int m, const float *grad_out, const int *max_idx, float *grad_points);
class GroupMaxpoolGradGpuOp: public OpKernel{
    public:
        explicit GroupMaxpoolGradGpuOp(OpKernelConstruction * context):OpKernel(context){}

        void Compute(OpKernelContext * context) override {
            const Tensor& points_tensor=context->input(0);
            OP_REQUIRES(context, points_tensor.dims()==3, errors::InvalidArgument("GroupMaxpoolGrad expects (batch_size, num_points, channel) points shape"));
            int b = points_tensor.shape().dim_size(0);
            int n = points_tensor.shape().dim_size(1);
            int c = points_tensor.shape().dim_size(2);

            const Tensor& max_idx_tensor=context->input(1);
            OP_REQUIRES(context,max_idx_tensor.dims()==3 && max_idx_tensor.shape().dim_size(0)==b, errors::InvalidArgument("GroupMaxpoolGrad expects (batch_size, npoints, channel) idx shape"));
            int m = max_idx_tensor.shape().dim_size(1);

            const Tensor& grad_out_tensor=context->input(2);
            OP_REQUIRES(context,grad_out_tensor.dims()==3 && grad_out_tensor.shape().dim_size(0)==b && grad_out_tensor.shape().dim_size(1)==m && grad_out_tensor.shape().dim_size(2)==c, errors::InvalidArgument("GroupMaxpoolGrad expects (batch_size, npoints, channel) grad_out shape"));
            ...
            const float *points = &(points_flat(0));
            auto max_idx_flat = max_idx_tensor.flat<int>();
            const int *max_idx = &(max_idx_flat(0));
            auto grad_out_flat = grad_out_tensor.flat<float>();
            const float *grad_out = &(grad_out_flat(0));
            auto grad_points_flat = grad_points_tensor->flat<float>();
            float *grad_points = &(grad_points_flat(0));
            cudaMemset(grad_points, 0, sizeof(float)*b*n*c);
            groupMaxpoolGradLauncher(b,n,c,m,grad_out,max_idx,grad_points);
        }
};
REGISTER_KERNEL_BUILDER(Name("GroupMaxpoolGrad").Device(DEVICE_GPU),GroupMaxpoolGradGpuOp);
